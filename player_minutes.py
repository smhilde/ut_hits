...
# Compile by event time
(raw.groupby(['event', 'before_break', 'day', 'event_type'], as_index=False)[['h2', 'h3', 'h4', 'h5']]
    .sum()
    ...
    .assign(n34=lambda x: x.sum34/x.player_min)).to_csv('compiled_by_event_time.csv', index=False)
...
(raw.groupby(['event', 'before_break', 'day', 'event_type'], as_index=False)[['h2', 'h3', 'h4', 'h5']]
    ...
    .assign(sum24=lambda x: x.h2+x.h3+x.h4)
    .assign(sum34=lambda x: x.h3+x.h4)
    .groupby(['event_type', 'before_break'], as_index=False)
    .sum()
    .assign(n2=lambda x: x.h2/x.player_min)
    .assign(n3=lambda x: x.h3/x.player_min)
    .assign(n4=lambda x: x.h4/x.player_min)
    .assign(n5=lambda x: x.h5/x.player_min)
    .assign(n24=lambda x: x.sum24/x.player_min)
    .assign(n34=lambda x: x.sum34/x.player_min)).to_csv('compiled_by_eventtype_time2.csv')